...
ComputeModeHf::ComputeModeHf(LAMMPS *lmp, int narg, char **arg) :
  ...
  id_ke(NULL), id_pe(NULL), id_stress(NULL)
{
  ...
  fh_debug = fopen("D_MODE_HF", "w");

}

/* ---------------------------------------------------------------------- */

ComputeModeHf::~ComputeModeHf()
{
  delete [] id_ke;
  delete [] id_pe;
  delete [] id_stress;
...
void ComputeModeHf::compute_vector()
{
