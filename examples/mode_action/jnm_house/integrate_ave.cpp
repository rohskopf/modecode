...
int main(int narg, char **arg){
  ...
  dt = 5.0*pow(10,-15);     //s timestep between each datapoint in Q.txt, so this is 5 fs between each timestep.
  ...
  prefactor = 5e-4; //dt, which has units of ps here.
  //double convert_data = 2.757518899e-34;
  double convert_data = 1.0;
  ...
  for (t=0;t<L;t++)
    for (j=0;j<Nmodes;j++)
      cor_data_ave[t][j] = cor_data_ave[t][j]*convert_data;
